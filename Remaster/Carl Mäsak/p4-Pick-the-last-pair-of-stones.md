...
*Originally published on [15 February 2011](http://strangelyconsistent.org/blog/p4-pick-the-last-pair-of-stones) by Carl Mäsak.*

*With the Yapsi release over with, and the two [preparatory](http://strangelyconsistent.org/blog/the-thing-with-nim) [posts](http://strangelyconsistent.org/blog/that-is-so-octal) done, let's get back to summarizing the results of the [Raku coding context](Masaks-raku-coding-contest.html). Here we go.*
...
In the *Star Trek* universe, there's a Starfleet training exercise called [Kobayashi Maru](https://en.wikipedia.org/wiki/Kobayashi_Maru), a simulation specially designed so that the cadet will fail. Quoting Wikipedia:

> There is no way to win the resulting battle, especially since the computer is allowed to "cheat" to guarantee defeat; the simulation ends with the understanding that the cadet's ship has been lost with all hands. The objective of the test is not for the cadet to outfight the opponent but rather to test the cadet's reaction to a no-win situation.
...
There were a number of things that I *didn't* say, but which we now know, especially after the two [preparatory](The-thing-with-nim.html) [posts](That-is-so-octal.html):
...
So if we were to, say, [pit all the five non-trivial solutions against each other](https://gist.github.com/masak/824207) in all possible combinations and with a number of different values of `$N`, we could pick up the places where the algorithms *deviate* from perfect play.
